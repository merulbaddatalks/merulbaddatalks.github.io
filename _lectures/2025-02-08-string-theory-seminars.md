---
title: "String Theory Seminars"
date: 2025-02-08
speaker: "String group"
affiliation: "BRAC University"
mode: "in-person/online"
abstract: >
  A seminar series on string theory hosted at BRAC University. We will build foundational intuition and
  mathematical tools for modern string theory.

sessions:
 - number: 1
   title: "History of String Theory"
   speaker: "Sirajush Salekin"
   recording: "https://youtu.be/qdRT3J3Y6BQ"
   
 - number: 2
   title: "Nambu Goto and Polyakov actions"
   speaker: "Ahmed Rakin Kamal"
   recording: "https://youtu.be/AHdlq_zkijE"
   notes: "https://drive.google.com/file/d/1grmxUb1DH2oqoWjCzQhYETM1iJBVgPlu/view?usp=drive_link"
 - number: 3
   title: "Lightcone coordinates and mode decomposition"
   speaker: "Ahmed Rakin Kamal"
   recording: "https://youtu.be/DDNU4ittzpo"
   notes: "https://drive.google.com/file/d/1d5ASfmMegczqZHPkS17iOKCl87GxQIsv/view?usp=sharing"
 - number: 4
   title: "Review and setting up for quantization"
   speaker: "Mishaal Hai"
   recording: "https://youtu.be/QxRW4QU3mQ0"
   notes: "https://drive.google.com/file/d/1BVoM_y6cv1wWcguGwBjuVmtMbypVEEQM/view?usp=sharing"
 - number: 5
   title: "Review and setting up for quantization (cont.)"
   speaker: "Mishaal Hai"
   recording: "https://youtu.be/_3-wID6-ZVQ"
   notes: "https://drive.google.com/file/d/1KfD__oLAxXU2LOqnEguriduVQUOsCByt/view?usp=sharing"
   


   
---

## Reference
The primary references for the series are *String Theory in a Nutshell* by Elias Kiritsis, David Tong's [String Theory](https://www.damtp.cam.ac.uk/user/tong/string.html) notes and String Theory and M-Theory: A Modern Introduction by Katrin Becker, Melanie Becker, and John H. Schwarz.

## How to Join
If you would like to join the seminars, please email [merulbaddatalks@gmail.com](mailto:merulbaddatalks@gmail.com).
